...
// ─── Tier colour tokens ────────────────────────────────────────────────────────
export const TIER_META: Record<SeatTier, {
  label: string
  bg: string
  border: string
  text: string
...
function buildSleeperFares(opts: {
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    const base =
      i <= 5 ? opts.lowerFront : i <= 14 ? opts.lowerMid : opts.lowerRear
    seats.push({
      seatId: `L${i}`,
      label: `L${i}`,
      position: "lower",
      ...
      festivalFare: Math.round(base * 1.3),
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    const base =
      i <= 5 ? opts.upperFront : i <= 14 ? opts.upperMid : opts.upperRear
    seats.push({
      seatId: `U${i}`,
      label: `U${i}`,
      position: "upper",
...
function buildSeaterFares(opts: {
  ...
  for (let r = 0; r < ROWS; r++) {
    ...
    const winBase =
      zone === "front"
        ...
        : zone === "mid"
          ? opts.windowMid
          : opts.windowRear
    const winTier: SeatTier =
      zone === "front" ? "premium" : zone === "mid" ? "standard" : "budget"
    ...
    ;[
      {
        id: `${row}1`,
        position: "window" as SeatPosition,
        base: winBase,
        tier: winTier,
      },
      {
        id: `${row}2`,
        position: "aisle" as SeatPosition,
        base: aisleBase,
        tier: aisleTier,
      },
      {
      ...
      seats.push({
        ...
        lastMinFare: Math.round(s.base * 0.9),
      })
    })
  }
  return seats
...
// ─── Lookup helpers ────────────────────────────────────────────────────────────
export function getSeatFare(
  config: RouteFareConfig,
  seatId: string,
  mode: "base" | "weekend" | "festival" | "lastMin" = "base",
): number {

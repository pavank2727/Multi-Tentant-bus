...
export default function BusListing({
  ...
  if (filters.ac)
    results = results.filter(
      (b) => b.busType.includes("AC") && !b.busType.includes("Non-AC"),
    )
  if (filters.nonAc)
    results = results.filter((b) => b.busType.includes("Non-AC"))
  if (filters.sleeper)
    results = results.filter((b) => b.busType.includes("Sleeper"))
  if (filters.seater)
    results = results.filter((b) => b.busType.includes("Seater"))
  ...
  return (
    <div style={{ background: "#F8FAFC", minHeight: "100%" }}>
      ...
      <div className="flex">
        {/* Filter sidebar */}
        <aside
          className="w-52 shrink-0 bg-white p-4"
          style={{
          ...
          {[
            {
              ...
              items: [
                ...
                { key: "nonAc" as const, label: "Non-AC" },
              ],
            },
            {
              title: "Seat Type",
              items: [
                { key: "sleeper" as const, label: "Sleeper" },
                { key: "seater" as const, label: "Seater" },
              ],
            },
          ].map((group) => (
            <div key={group.title} className="mb-5">
              <div
                className="font-mono text-xs uppercase tracking-wider mb-2.5"
                style={{ color: "#94A3B8" }}
              >
                {group.title}
              </div>
          ...
          {results.map((bus, i) => (
            <div key={i} className="card card-hover rounded-xl p-5">
              <div className="flex items-start justify-between">
                ...
                {/* Price + CTA */}
                <div
                  className="ml-6 text-right flex flex-col items-end justify-between"
                  style={{ minWidth: 140 }}
                >
                  <div>
                    <div
                  ...
                  <div className="mt-4">
                    <div
                      ...
                      style={{
                        color: bus.availableSeats <= 5 ? "#F97316" : "#94A3B8",
                      }}
                    >

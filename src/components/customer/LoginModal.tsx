  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,23,42,0.5)", backdropFilter: "blur(6px)" }}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
        {/* Header */}
        <div className="px-6 py-5 flex items-start justify-between" style={{ background: "#F97316" }}>
          <div>
            <div className="mb-1">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.2)", color: "#fff", fontSize: 18 }}>×</button>
        </div>
        ...
        <div className="p-6">
          {step === "phone" ? (
            <>
              ...
              <p className="text-center text-xs mt-3" style={{ color: "#94A3B8" }}>By continuing, you agree to our Terms of Service</p>
            </>
          ) : (
            <>
              <div className="text-center mb-5">
                <div className="font-jakarta font-semibold mb-1" style={{ color: "#0F172A" }}>Enter OTP</div>
                <div className="text-sm" style={{ color: "#94A3B8" }}>Sent to +91 {phone.slice(0,5)} {phone.slice(5)}</div>
              </div>
              ...
              <div className="text-center mb-4">
                {canResend
                  ? <button onClick={() => setStep("otp")} className="font-medium text-sm" style={{ color: "#F97316" }}>Resend OTP</button>
              ...
              <button onClick={() => setStep("phone")} className="w-full text-center text-sm mt-3" style={{ color: "#94A3B8" }}>← Change number</button>
            </>
          )}
        </div>
      </div>

...
function assignStatuses(
  ...
  const fareMap = Object.fromEntries(
    fareConfig?.seats.map((s) => [s.seatId, s]) ?? [],
  )
  ...
  return seats.map((s, i) => {
    ...
    const status: BookingStatus =
      h < 3 ? "booked" : h === 3 ? "female" : "available"

    const fareEntry = fareMap[s.label]
    const fare =
      fareEntry?.baseFare ??
      (s.type === "lower"
        ? Math.round(baseFare * 1.0)
        : s.type === "upper"
          ? Math.round(baseFare * 0.87)
          : baseFare)
...
function CustSeaterCell({
  ...
  return (
    <div
      ...
      title={`${seat.label} · ₹${seat.fare}${
        seat.status === "female" ? " · Ladies only" : ""
    ...
    >
      ...
      <div
        style={{
          ...
          height: 22,
          background:
            seat.status === "selected" ? "rgba(255,255,255,0.15)" : "#fff",
...
function CustBerthCell({
  ...
  return (
    <div
      onClick={interactive ? onClick : undefined}
      title={`${seat.label} · ${seat.type} berth · ₹${seat.fare}${
        seat.status === "female" ? " · Ladies only" : ""
      }${seat.status === "booked" ? " · Booked" : ""}`}
    ...
    >
      ...
      <div
        style={{
          ...
          height: 16,
          background:
            seat.status === "selected" ? "rgba(255,255,255,0.25)" : "#fff",
          border: `1.5px solid ${col.border}`,
          borderRadius: 5,
        }}
      />
      ...
      {/* Female badge */}
      {seat.status === "female" && (
        <div style={{ position: "absolute", top: 3, right: 3, fontSize: 8 }}>
          ♀
        </div>
      )}
    </div>
...
function DesignedBusMap({
  ...
  const { busMode, rows, leftCols, rightCols, driverSide, entryPos, exitPos } =
    layout
  ...
  const renderSlot = (row: number, col: number, side: BusSide) => {
    ...
    return (
      <div key={key} className="flex gap-1">
        {lower ? (
          <CustBerthCell seat={lower} onClick={() => onToggle(lower.id)} />
        ) : (
          <div style={{ width: 46, height: 86 }} />
        )}
        {upper ? (
          <CustBerthCell seat={upper} onClick={() => onToggle(upper.id)} />
        ) : (
          <div style={{ width: 46, height: 86 }} />
        )}
      </div>
    ...
    >
      ...
      <div className="flex items-center mb-1.5" style={{ paddingLeft: 26 }}>
        ...
        <div
          ...
          style={{
            ...
            color: "#94A3B8",
            width:
              leftCols * (busMode === "seater" ? 46 : 96) + (leftCols - 1) * 6,
          }}
        >
          ← LEFT
        </div>
        ...
        <div
          ...
          style={{
            ...
            color: "#94A3B8",
            width:
              rightCols * (busMode === "seater" ? 46 : 96) +
              (rightCols - 1) * 6,
          }}
        >
          RIGHT →
      ...
      <div className="space-y-2">
        {Array.from({ length: rows }, (_, r) => {
          const rowLabel =
            busMode === "seater" ? String.fromCharCode(65 + r) : String(r + 1)
          ...
          return (
            <div key={r} className="flex items-center gap-1">
              ...
              >
                <div
                  style={{ width: 1, height: "100%", background: "#E2E8F030" }}
                />
              </div>
              {/* Right seats */}
      ...
      <div className="text-center mt-3">
        <span
          className="font-mono"
          style={{ fontSize: 8, color: "#CBD5E1", letterSpacing: "0.15em" }}
        >
          REAR →
        </span>
      </div>
...
function DoorBadge({ kind }: { kind: "entry" | "exit" }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="font-mono"
        style={{ fontSize: 7, fontWeight: 800, color, letterSpacing: "0.1em" }}
      >
        {kind === "entry" ? "ENTRY" : "EXIT"}
      </div>
    </div>
...
function buildFallbackSeats(busId: string): FallbackSeat[] {
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    seats.push({
      id: lo,
      label: lo,
      status: prebooked.includes(lo)
        ? "booked"
        : female.includes(lo)
          ? "female"
          : "available",
      deck: "lower",
      fareEntry: fareMap[lo],
    })
    seats.push({
      id: up,
      label: up,
      status: prebooked.includes(up)
        ? "booked"
        : female.includes(up)
          ? "female"
          : "available",
      deck: "upper",
      fareEntry: fareMap[up],
    })
  }
  return seats
}

function FallbackSeatCell({
  seat,
  onClick,
}: {
  seat: FallbackSeat
  ...
  const STATUS_COLORS: Record<SeatStatus, {
    bg: string
    border: string
    text: string
  }> = {
  ...
  return (
    <div
      ...
      style={{
        ...
        width: 56,
        height: 48,
        transform: seat.status === "selected" ? "scale(1.06)" : "scale(1)",
        boxShadow:
      ...
      {fare && (
        <span
          style={{
            ...
            marginTop: 2,
            opacity: 0.75,
            color: tier ? TIER_META[tier].text : c.text,
          }}
        >
          ₹{fare}
        </span>
...
export default function SeatSelection({
  ...
  const [custSeats, setCustSeats] = useState<CustomerSeat[]>(() =>
    designedLayout
      ? assignStatuses(designedLayout.seats, bus.busNumber, bus.fare)
      : [],
  )
  ...
  const toggle = (id: string) => {
    if (designedLayout) {
      ...
      if (selected.includes(id)) {
        ...
        setCustSeats((p) =>
          p.map((s) => (s.id === id ? { ...s, status: "available" } : s)),
        )
      } else if (selected.length < 6) {
        if (selected.length === 0) setTimer(300)
        setSel((p) => [...p, id])
        setCustSeats((p) =>
          p.map((s) => (s.id === id ? { ...s, status: "selected" } : s)),
        )
      }
    } else {
  ...
  const fareMap = useMemo(() => {
    ...
    return Object.fromEntries(
      fareConfig?.seats.map((s) => [s.seatId, s.baseFare]) ?? [],
    )
  ...
  return (
    <div style={{ background: "#F8FAFC", minHeight: "100%" }}>
      {/* Header */}
      <header
        className="bg-white px-6 py-3.5 flex items-center justify-between sticky top-0 z-10"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="font-jakarta font-bold" style={{ color: "#0F172A" }}>
            {bus.operator} · {bus.busName}
          </div>
      ...
      <div className="flex gap-5 max-w-6xl mx-auto p-5">
              ...
              <>
                ...
                <div className="flex gap-2 mb-4">
                  {(["lower", "upper"] as const).map((d) => (
                    <button
                      ...
                      style={{
                        background: fbDeck === d ? "#F97316" : "#fff",
                        color: fbDeck === d ? "#fff" : "#64748B",
                        border: `1.5px solid ${
                          fbDeck === d ? "#F97316" : "#E2E8F0"
                        }`,
                      }}
                    >
                ...
                >
                  ...
                  <span
                    ...
                    style={{ color: "#94A3B8" }}
                  >
                    {fbDeck.toUpperCase()} BERTHS
                  </span>
                </div>
                <div className="space-y-2">
                  {Array.from({ length: 10 }, (_, ri) => {
                    const row = fbSeats
                      .filter((s) => s.deck === fbDeck)
                      .slice(ri * 2, ri * 2 + 2)
                    return (
          ...
          {!designedLayout && (
            <div className="mt-3 text-center">
              <span className="font-mono text-xs" style={{ color: "#CBD5E1" }}>
                Design a layout in the Operator Portal → Seat Designer to see a
                custom bus map here
              </span>
            </div>
          )}
        ...
        <div className="shrink-0" style={{ width: 272 }}>
          <div className="card rounded-xl p-5 sticky top-24">
            <div
              className="font-jakarta font-bold mb-4"
              style={{ color: "#0F172A" }}
            >
              Booking Summary
            </div>
            ...
            {selected.length === 0 ? (
              ...
              <>
                  ...
                  {selectedSeatsData.map((s) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          background: m ? m.bg : "#F8FAFC",
                          border: `1px solid ${m ? m.border : "#F1F5F9"}`,
                        }}
                      >
                        <div className="flex items-center gap-2">
                          {m && (
                          ...
                          <div>
                            ...
                            {entry && (
                              <div
                                className="font-mono"
                                style={{ fontSize: 10, color: m!.text }}
                              >
                ...
                <button
                  onClick={() =>
                    onBook(
                      selected,
                      Object.fromEntries(
                        selectedSeatsData.map((s) => [s.id, s.fare]),
                      ),
                    )
                  }
                ...
                >
                  Seats locked for{" "}
                  <span style={{ color: "#D97706" }}>{fmt(timer)}</span>
                </div>
              </>
            )}
          </div>
